...
export default class HomePage {
      ...
      <article class="story-card">
        ...
        <div class="story-info">
          ...
          <div class="story-meta">
            <span class="story-location">${story.lat && story.lon ? `📍 Lat: ${story.lat.toFixed(2)}, Lon: ${story.lon.toFixed(2)}` : '📍 Location not available'}</span>
            <span class="story-date">${story.createdAt ? new Date(story.createdAt).toLocaleDateString('id-ID', { 
              day: 'numeric', 
              month: 'short', 
              year: 'numeric' 
            }) : 'Unknown date'}</span>
  ...
  _initializeMap() {
    ...
    this.stories.forEach(story => {
      if (story.lat && story.lon) {
        ...
        const popupContent = `
          <div class="map-popup">
            <img src="${story.photoUrl}" alt="${story.name || 'Story image'}" style="width: 100%; max-width: 200px; height: auto; border-radius: 4px;"/>
  ...
  _initializeViewControls() {
    ...
    listViewBtn.addEventListener('click', () => {
      this._switchView('list', listViewBtn, mapViewBtn, storiesListDiv, mapContainerDiv);
    });

    mapViewBtn.addEventListener('click', () => {
      this._switchView('map', listViewBtn, mapViewBtn, storiesListDiv, mapContainerDiv);
    });
  }
